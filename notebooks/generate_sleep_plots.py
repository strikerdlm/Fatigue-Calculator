#!/usr/bin/env python3
"""
Sleep Restriction Analysis - Plot Generation Script
Generates comprehensive visualizations for sleep restriction effects on alertness
"""
...
def main():
    ...
    plt.tight_layout()
    plt.savefig('notebooks/sleep_restriction_main_plots.png', dpi=300, bbox_inches='tight')
    print("Main plots saved to: notebooks/sleep_restriction_main_plots.png")
    ...
    for sleep_duration in SLEEP_DURATIONS:
        ...
        risk_data.append([critical, high, moderate, optimal])
    
    risk_df = pd.DataFrame(risk_data, 
                          columns=['Critical (<70%)', 'High (70-80%)', 'Moderate (80-90%)', 'Optimal (≥90%)'],
                          index=[f'{sd}h' for sd in SLEEP_DURATIONS])
    
    # Create stacked bar chart
    risk_df.plot(kind='bar', stacked=True, ax=ax, 
                color=['red', 'orange', 'yellow', 'green'], alpha=0.8)
    ax.set_title('Performance Risk Analysis by Sleep Duration', fontsize=14, fontweight='bold')
    ax.set_xlabel('Sleep Duration')
    ax.set_ylabel('Percentage of Work Time')
    ax.legend(title='Risk Level', bbox_to_anchor=(1.05, 1), loc='upper left')
    ax.grid(True, alpha=0.3, axis='y')
    
    plt.tight_layout()
    plt.savefig('notebooks/sleep_restriction_risk_analysis.png', dpi=300, bbox_inches='tight')
    print("Risk analysis plot saved to: notebooks/sleep_restriction_risk_analysis.png")
    ...
    # Save data
    df.to_csv('notebooks/sleep_restriction_results.csv', index=False)
    print(f"\nData saved to: notebooks/sleep_restriction_results.csv")
    
    print("\n" + "="*60)
    print("ANALYSIS COMPLETE - All plots and data saved!")
    print("="*60)
